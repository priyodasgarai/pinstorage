...
<div class="row wrapper border-bottom white-bg page-heading">
   ...
   <div class="col-sm-8">
      <div class="title-action">
         <a href="{{url('category-management/add')}}" class="btn btn-primary"><i class="fa fa-plus"></i>
         	Add Category
         </a>
      </div>
   </div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#searchToDate").datepicker({ maxDate:0}); 
    		// minDate: "+1M +10D" 
    	let searchCategory=searchFormDate=searchToDate = "";
		var dataTable = $('#categoryTable').DataTable({
            ...
            searching:  false,
            ajax: {
		        url: baseUrl+'category-management/ajax-table',
		        type:'POST',
		        data: function(data){
			...
			buttons: [
			    {
		            extend: 'excelHtml5',
		            text: 'Export Excel ',
		            filename: function () { return 'Category List ' + new Date(); },
		            exportOptions: {
		                columns: [0, 1, 2, 3, 4, 5],
		                modifier: {
		                    order: 'current',                       
		...
		$('#searchCategory').keydown(function(){
		    dataTable.draw();
		});
